...
module.exports = function(context) {
  ...
  // Cu.import is deprecated. Please use
  ...
  // Cc[...].getService() is deprecated. Please use
  ...
  // XPCOMUtils.defineLazyGetter(this, "_strings", ...) is deprecated.
  // Please use loader.lazyGetter(this, "_strings", () => { return ...; });
  function lazyGetterMatched(scope, name, context, node) {
    context.report(node, "XPCOMUtils.defineLazyGetter(" + scope + ", \"" +
                         name + "\", ...) is deprecated. Please use " +
                         "loader.lazyGetter(" + scope + ", \"" + name +
                         "\", () => { return ...; });");
  }
  ...
  function lazyModuleGetterMatched(scope, name, path, context, node) {
    context.report(node, "XPCOMUtils.defineLazyModuleGetter(" + scope + ", \"" +
                         name + "\", ...) is deprecated. Please use " +
                         "loader.lazyImporter(" + scope + ", \"" + name +
                         "\", \"" + path + "\"");
  }
  ...
  return {
    ExpressionStatement: function(node) {
      ...
      matches =
        source.match(/^XPCOMUtils\.defineLazyModuleGetter\((\w+), "(\w+)",\s\"([\w\:\/]+\.jsm?)\"\);?$/);
      if (matches) {
        var scope = matches[1];
        name = matches[2];
        path = matches[3];
    ...
    VariableDeclaration: function(node) {
      ...
      var matches =
        source.match(/(\w+)\s=\s(?:Cu|Components\.utils)\.import\("(.*\/(.*?)\.jsm?)"\);?$/);
      if (matches) {
        var name = matches[1];
        var path = matches[2];
      ...
      matches =
        source.match(/(\w+)\s=\s(?:Cc|Components\.classes)\[\"(.*?)\"\]\.getService\((?:Ci|Components\.interfaces)\.(\w+)\);?$/);
      if (matches) {
        name = matches[1];
